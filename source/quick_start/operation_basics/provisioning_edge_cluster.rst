...
An edge cluster is a group of resources in OpenNebula, and their corresponding resources in AWS, that are suitable for running at edge locations with a minimal footprint.

OpenNebula provides a ready-to-use specification of the cluster. During provision, OpenNebula creates all of the cluster’s resources and, with the aid of Terraform, their corresponding objects on AWS.
...
The following resources are created in OpenNebula:
    ...
    * **Datastore**: each provision deploys two datastores, the system and the image. Datastores for edge clusters are based on :ref:`OneStor datatores <onestor_ds>`; datastores for HCI clusters are based on Ceph.
...
To add a new provider, click the **Plus button** on the right:

.. image:: /images/6.10-OneProvision-add_provider.png
    :align: center
    :scale: 70%
...
To provision the cluster, open the left-hand pane, select **Provisions**, and click the **Plus icon** on the right:

.. image:: /images/6.10-oneprovision-new_provision.png
    :align: center
    :scale: 80%
...
OneProvision should display the **Provider** screen. Click the **aws-frankfurt** box, then click **Next**.

.. image:: /images/6.10-oneprovision-provider.png
    :align: center
    :scale: 80%
...
.. image:: /images/6.10-oneprovision-edge_cluster_inputs.png
    :align: center
    :scale: 70%
