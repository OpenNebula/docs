...
You can always evaluate OpenNebula by following our :ref:`Quick Start Guide <quick_start>`, where you can learn how to quickly and easily deploy an OpenNebula Front-end on AWS using :ref:`miniONE <try_opennebula_on_kvm>`, then to provision an Edge Cluster to run :ref:`Virtual Machines <running_virtual_machines>` and an enterprise-grade :ref:`Kubernetes Cluster <running_kubernetes_clusters>` on a truly multi-cloud environment. This is the fastest way for you to familiarize yourself with the new OpenNebula, as you only need the necessary resources to deploy the OpenNebula front-end.
