...
Since Firecracker is very lightweight and optimized, the amount of resources dedicated to the hypervisor are much lower when compared to KVM. There is no exact figure but you can get an idea from :ref:`the microVM density tests result <hv_scalability>`.
