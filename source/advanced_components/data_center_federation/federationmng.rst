...
Users can switch Zones through the command line using the `onezone </doc/5.8/cli/onezone.1.html>`__ command. The following session can be examined to understand the Zone management through the CLI.
...
We can see in the above command output that the user has access to both "OpenNebula" and "ZoneB", and it is currently in the "OpenNebula" Zone. The active Zone can be changed using the 'set' command of `onezone </doc/5.8/cli/onezone.1.html>`__:
