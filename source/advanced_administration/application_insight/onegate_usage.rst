...
detect problems in their applications, and trigger OneFlow elasticity
rules.
...
Machines. OpenNebula assigns an individual token to each VM instance,
and Applications running inside the VM use this token to send monitoring
metrics to OneGate.
...
First, the cloud administrator must configure and start the `OneGate
server </./onegate_configure>`__.

Setup the VM Template
---------------------

Your VM Template must set the CONTEXT/TOKEN attribute to â€œyesâ€?.
...
When this Template is instantiated, OpenNebula will automatically add
the ONEGATE\_URL context variable, and a token.txt will be placed in the
context cdrom. This token.txt file is only accessible from inside the
VM.
...
Monitoring values, in the usual â€œATTRIBUTE = VALUEâ€? OpenNebula
syntax
