...
Read the :ref:`Compatibility Guide <compatibility>` and `Release Notes <http://opennebula.org/software/release/>`_ to know what is new in OpenNebula 5.2.

Upgrading a Federation
================================================================================

If you have two or more 5.2.x OpenNebulas working as a :ref:`Federation <introf>`, you can upgrade each one independently. Zones with an OpenNebula from the 5.2.x series can be part of the same federation, since the shared portion of the database is compatible.
...
It is highly recommended **not to keep** your current ``oned.conf``, and update the ``oned.conf`` file shipped with OpenNebula 5.2 to your setup. If for any reason you plan to preserve your current ``oned.conf`` file, read the complete :ref:`oned.conf reference for 5.2 <oned_conf>`.
