...
-  :ref:`Disaggregated Data Centers <ddc_overview>` provides tools to build and grow the physical infrastructure with resources from public bare-metal cloud providers, e.g. :ref:`Amazon EC2 <ddc_provision_driver_ec2>` and :ref:`Packet <ddc_provision_driver_packet>`. New physical machines are allocated from the provider, configured to run the hypervisor and added into the OpenNebula as new hosts, datastores and virtual networks.
