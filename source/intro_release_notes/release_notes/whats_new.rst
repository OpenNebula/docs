...
This OpenNebula release is named after the edges of nebulas. Nebulas are diffuse objects, and their edges can be considered vacuum. However, they are very thick, so they appear to be dense. This is the aim of OpenNebula 5.8, to provide computing power on a wide geographic surface to offer services closer to customers, building a cloud managed from a single portal over very thin infrastructure. There's an `Edge Nebula <ihttp://freelancer.wikia.com/wiki/Edge_Nebula>` on the Freelancer videogame.
...
- `Fix issue that prevents to update image metadata while in LOCKED state <https://github.com/OpenNebula/one/issues/2770>`__.
- `Fix issue with Bundler installation with old Ruby <https://github.com/OpenNebula/one/issues/2778>`__.
