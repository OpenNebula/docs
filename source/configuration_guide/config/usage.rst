...
By default, patch process finishes successfully even if all changes were not applied. We can distinguish between finished full or partial application by checking the exit code of the command. We can also request to apply all or none changes by using ``--all`` argument.
...
Subcommands ``diff`` and ``patch`` can be chained to apply changes from one front-end to another front-end:
